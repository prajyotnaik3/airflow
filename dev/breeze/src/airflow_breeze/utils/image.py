...
from airflow_breeze.utils.parallel import ParallelMonitor, check_async_run_results, get_output_files
...
def run_pull_in_parallel(
    ...
    poll_time = 10.0
    outputs = get_output_files([f"Image {image_params.python}" for image_params in image_params_list])
    m = ParallelMonitor(outputs=outputs)
    m.start()
    try:
        if not verify:
            with ci_group(f"Pull for {python_version_list}"):
            ...
            with ci_group(f"Pull/verify for {python_version_list}"):
                get_console().print(f"[info]Parallelism: {parallelism}")
                results = [
                    pool.apply_async(

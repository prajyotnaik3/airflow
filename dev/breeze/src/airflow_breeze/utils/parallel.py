...
DOCKER_PROGRESS_MATCHER = re.compile(r'\s*#(\d*) ')


def progress_method_docker_buildx(output: Output) -> Optional[str]:
    last_lines, last_lines_no_colors = get_last_lines_of_file(output.file_name, num_lines=3)
    best_progress: int = 0
    best_line: Optional[str] = None
    for index, line in enumerate(last_lines_no_colors):
        match = DOCKER_PROGRESS_MATCHER.match(line)
        if match:
            docker_progress = int(match.group(1))
            if docker_progress > best_progress:
                best_progress = docker_progress
                best_line = line
    return f"Progress: [info]{output.title:<20}[/] {best_line}"
...
class ParallelMonitor(Thread):
    ...
    def print_single_progress(self, output: Output):
        if self.progress_method:
            ...
            if progress:
                get_console().print(self.progress_method(output))
                return
        size = os.path.getsize(output.file_name) if Path(output.file_name).exists() else 0
        get_console().print(f"Progress: [info]{output.title:<70} {size:>93} bytes")
    ...
    def cancel(self):
        get_console().print("[warning]Cancelling progress monitoring.")
        self.cancelled = True
        try:
    ...
    def run(self):
        try:
            while not self.cancelled:
                self.print_summary()
                with self.condition:
                    self.condition.wait(self.time_in_seconds)
        except Exception:

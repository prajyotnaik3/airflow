...
def run_build_in_parallel(
    ...
    with ci_group(f"Building for {python_version_list}"):
        ...
        outputs = get_output_files([f"CI {image_params.python}" for image_params in image_params_list])
        m = ParallelMonitor(outputs=outputs, progress_method=progress_method_docker_buildx)
        m.start()
        try:
            results = [
                pool.apply_async(
